...
func runDashboard(ctx context.Context, cfg bot.Config, once, force bool, run runFunc, input, output *os.File) error {
	d := newDashboard(cfg)
	...
	err = func() error {
		...
		return dashboardSession(ctx, d, once, force, run, input, output)
	}()
	_, writeErr := fmt.Fprint(output, d.summary(err))
	return errors.Join(err, writeErr)
}

func dashboardSession(ctx context.Context, d *dashboard, once, force bool, run runFunc, input, output *os.File) error {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
	log := slog.New(&dashboardHandler{d: d})
	result := make(chan error, 1)
	go func() { result <- run(bot.WithProgress(ctx, d.update), d.cfg, log, once, force) }()
