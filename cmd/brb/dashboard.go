...
func newDashboard(cfg bot.Config) *dashboard {
	...
	return &dashboard{cfg: cfg, started: now, phaseStarted: now, lastActivity: now,
		progress: bot.Progress{Phase: "starting", Task: "Loading saved release progress"}}
}
...
func (d *dashboard) update(p bot.Progress) {
	...
	// Keep a release open while new results arrive; the overview follows new
	// releases only when the user has not browsed away from the newest one.
	if (d.detail || d.selected > 0) && len(d.progress.Releases) > d.selected {
		id := d.progress.Releases[len(d.progress.Releases)-1-d.selected].ID
		for i, f := range p.Releases {
			if f.ID == id {
				d.selected = len(p.Releases) - 1 - i
				break
			}
		}
	}
	d.progress = p
	d.selected = max(0, min(d.selected, len(p.Releases)-1))
}
...
func (d *dashboard) key(key string) bool {
	...
	case "enter":
		if len(d.progress.Releases) > 0 && d.view != 2 {
	...
	case "j", "down", "pgdown":
		if d.view == 2 {
			...
			d.selected = max(0, min(len(d.progress.Releases)-1, d.selected+1))
		}
	case "k", "up", "pgup":
		if d.view == 2 {
			d.scroll++
	...
	case "G":
		if d.view == 2 {
			...
			d.selected = max(0, len(d.progress.Releases)-1)
		}
	}
	return false
}
...
func (d *dashboard) render(width, height int, now time.Time, color bool) string {
	...
	add("RELEASE BOT  "+dashboardRepository(d.cfg), "1;36")
	if d.detail && len(p.Releases) > 0 {
		add(fmt.Sprintf("RELEASE DETAIL · %d/%d", d.selected+1, len(p.Releases)), "2")
		f := p.Releases[len(p.Releases)-1-d.selected]
	...
	lastTag := p.LastTag
	if lastTag == "" {
		lastTag = "none recorded"
	}
	add("Last release: "+lastTag, "1")
	if height >= 18 {
		changes := "Commit counts refresh on the next cadence check"
		if p.ChangesKnown {
			changes = fmt.Sprintf("%d unreleased commits · %d recent", p.Unreleased, p.Recent)
		}
		add(changes, "2")
	}
	...
	tabs := []string{"1 Overview", "2 Releases", "3 Activity"}
...
func (d *dashboard) releaseRows(limit int) []string {
	if limit <= 0 {
		return nil
	}
	releases := d.progress.Releases
	if len(releases) == 0 {
		if d.progress.Phase == "waiting" || d.progress.Phase == "complete" {
			return []string{"No releases in saved history."}
		}
		return []string{"No releases yet."}
	}
	d.selected = max(0, min(d.selected, len(releases)-1))
	start := max(0, d.selected-limit+1)
	var rows []string
	for i := start; i < min(len(releases), start+limit); i++ {
		f := releases[len(releases)-1-i]
		prefix := "  "
		if i == d.selected {
			prefix = "› "
		}
...
func (d *dashboard) summary(err error) string {
	...
	text := fmt.Sprintf("brb %s · %s · %d releases verified this run\n", cleanText(dashboardRepository(d.cfg)), status, d.completed)
	for _, f := range d.progress.Releases {
...
func dashboardRepository(cfg bot.Config) string {
	if repo := cfg.GitHubRepo(); repo != "" {
		return repo
	}
	return cfg.Remote
}
