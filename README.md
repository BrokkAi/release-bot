...
Unpushed commits on the watched branch in the bot's managed checkout also count as unreleased work. When the normal release cadence is due, preparation preserves those commits, pushes them through the repository's normal process, and prepares a new version. The bot never force-pushes or overwrites divergent history or unfinished edits. Commits in a separate development checkout are not imported automatically.
